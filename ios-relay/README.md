...
- `mkcert` (best UX; trusted if you install the CA on your iPhone):
  - `mkcert -cert-file .cert/cert.pem -key-file .cert/key.pem 192.168.154.10 localhost 127.0.0.1`
- `openssl` (local CA + server cert with SAN; install the CA on your iPhone to avoid warnings):
  ...
  - `openssl req -new -newkey rsa:2048 -nodes -keyout .cert/key.pem -out .cert/server.csr.pem -subj "/CN=192.168.154.10" -addext "subjectAltName=IP:192.168.154.10,IP:127.0.0.1,DNS:localhost" >/dev/null`
  - `printf "%s\n" "basicConstraints=critical,CA:FALSE" "keyUsage=critical,digitalSignature,keyEncipherment" "extendedKeyUsage=serverAuth" "subjectAltName=IP:192.168.154.10,IP:127.0.0.1,DNS:localhost" > .cert/server.ext`
...
- Enter the Python server IP/port (defaults match the hackathon server: `192.168.154.10:8765`)
